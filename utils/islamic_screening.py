...
def check_business_practices(info):
    ...
    if any(
        industry.lower() == alcohol_ind.lower() for alcohol_ind in alcohol_industries
    ):
    ...
    is_financial_institution = (
        ...
        or any(indicator in company_name for indicator in financial_indicators)
        or (
            sector == "financial services"
            and any(indicator in industry.lower() for indicator in financial_indicators)
        )
    )
...
def calculate_islamic_ratios(financial_data, info):
    ...
    try:
        ...
        return {
            ...
            **business_practices,
            "is_fully_compliant": (
                ratios["is_debt_compliant"]
                and ratios["is_liquidity_compliant"]
                and ratios["is_receivables_compliant"]
                and business_practices["is_business_compliant"]
            ),
        }
